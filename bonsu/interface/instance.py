...
def SetInstanceObject(panelphase, instance, idx, subpanel_items):
	...
	if subpanelname in subpanel_items:
		...
		if subpanelname == 'View Chopped':
			...
			panelphase.pipelineitems[idx].number_of_slices.value.SetValue(object[5])
			panelphase.pipelineitems[idx].theta.value.SetValue(object[6])
			panelphase.pipelineitems[idx].psi.value.SetValue(object[7])
			panelphase.pipelineitems[idx].separation.value.SetValue(object[8])
			panelphase.pipelineitems[idx].thickness.value.SetValue(object[9])
			panelphase.pipelineitems[idx].chkbox_axes.SetValue(object[10])
			try:
				panelphase.pipelineitems[idx].feature_angle.value.SetValue(object[11])
				panelphase.pipelineitems[idx].axes_fontfactor.value.SetValue(object[12])
			except Exception as e:
				print(f'While loading View Chopped, particularly FA and axes FF, {type(e)} - {e}')
			try:
				panelphase.pipelineitems[idx].meshsubiter.value.SetValue(object[13])
			except Exception as e:
